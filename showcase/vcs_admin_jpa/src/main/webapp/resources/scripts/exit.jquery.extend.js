			...
			.extend(
					jQuery,
					{
						...
						invoke : function(arr, methodName) {
							var ret = [], args = Array.prototype.slice.call(
									arguments, 2);
						...
						number : function(v, defaultValue) {
							v = Number($.isEmpty(v)
									|| $.isArray(v)
									|| typeof v == 'boolean'
									|| (typeof v == 'string' && v.trim().length == 0) ? NaN
									: v);
								...
								$.applyIf(option, {
									...
									success : function(responseText,
										...
										if ($.isFunction(option.successCall)) {
											option.successCall(responseText,
													statusText, xhr, $form)
										}
									}
								});
						...
						ellipsis : function(value, len, word) {
							if (value && value.length > len) {
								if (word) {
									var vs = value.substr(0, len - 2), index = Math
											.max(vs.lastIndexOf(' '), vs
													.lastIndexOf('.'), vs
													.lastIndexOf('!'), vs
													.lastIndexOf('?'));
			...
			.applyIf(
					Function.prototype,
					{
						createInterceptor : function(fcn, scope) {
							var method = this;
							return !$.isFunction(fcn) ? this
									: function() {
										var me = this, args = arguments;
										fcn.target = me;
										fcn.method = method;
										return (fcn.apply(
												scope || me || window, args) !== false) ? method
						...
						createDelegate : function(obj, args, appendArgs) {
							...
							return function() {
								...
								if (appendArgs === true) {
									callArgs = Array.prototype.slice.call(
											arguments, 0);
									...
									var applyArgs = [ appendArgs, 0 ]
											.concat(args);
									Array.prototype.splice.apply(callArgs,
											applyArgs)
								}
					...
					String.prototype,
					{
						...
						booleanValue : function(compareValue) {
							...
							return $.isNotEmpty(value)
									&& (value === "true" || value === "1" || ($
											.isNotEmpty(compareValue) || value == compareValue))
						}
					});

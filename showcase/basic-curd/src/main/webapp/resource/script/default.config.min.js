(function($){
	...
	initComponent = function(){
		...
		$(".collapse").collapse();
		$(".btn").button();
		$(".alert.auto-close").alert.defer(3000,$(".alert.auto-close"),["close"]);
		...
		$("form").each(function(i,o){
			...
			temp.find("button[type='reset']").click(function(){
				var inputs = temp.find("input:not(:disabled):not(:hidden)"),
				selects = temp.find("select:not(:disabled):not(:hidden)");
	...
	$.fn.maskload = function(url,data,callback) {
		return $.maskload($(this),url,data,callback);
	};
